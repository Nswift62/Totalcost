...
(функция (глобальная, заводская) {
	...
	if (typeof module === "object" && typeof module.exports === "object") {
		...
		// Для получения дополнительной информации см. Билет # 14549.
...
var flat = arr.flat? функция (массив) {
	return arr.flat.call (массив);
}: функция (массив) {
	return arr.concat.apply ([], массив);
};
...
// без охраны в другом месте, кажется безопаснее определять глобальные только для этого модуля
	...
	for (; i <длина; i ++) {
		...
		if ((options = arguments [i])! = null) {
			...
			для (имя в опциях) {
				...
				if (глубокий && copy && (jQuery.isPlainObject (copy) ||
...
jQuery.extend ({
	...
	map: function (elems, callback, arg) {
		...
		// Проходим массив, переводя каждый из элементов в их новые значения
		...
		// Пройдемся по каждому ключу на объекте,
		} еще {
			для (я в элементах) {
				значение = обратный вызов (elems [i], i, arg);
...
jQuery.each («Булево число Строка функции Массив Дата Символ объекта RegExp Ошибка» .split (""),
функция (_i, имя) {
...
я
	...
	booleans = "проверено | выбрано | асинхронно | автофокус | автозапуск | управление | отложить | отключено | скрыто |" +
		"ISMAP | петля | множественная | открыта | чтения | требуется | область действия",
	...
	attribute = "\\ [" + пробел + "* (" + идентификатор + ") (?:" + пробел +
		...
		// "Значения атрибута должны быть идентификаторами CSS [capture 5]
	...
	matchExpr = {
		...
		"РЕБЕНОК": новый RegExp ("^ :( только | первый | последний | последний | nth | последний-последний) - (потомок | типа) (?: \\ (" +
			пробел + "* (четное | нечетное | (([+ -] |) (\\ d *) n |)" + пробел + "* (?: ([+ -] |)" +
			пробел + "* (\\ d +) |))" + пробел + "* \\) |)", "i"),
		"bool": new RegExp ("^ (?:" + booleans + ") $", "i"),

		// Для использования в библиотеках, реализующих .is ()
		// Мы используем это для сопоставления POS в `select`
		«needsContext»: новый RegExp («^» + пробел +
	...
	funescape = function (escape, nonHex) {
		...
		вернуть nonHex?
			...
			// Заменить шестнадцатеричную escape-последовательность на кодированную кодовую точку Unicode
			// Поддержка: IE <= 11 +
			// Для значений вне Базовой многоязычной плоскости (BMP), вручную построить
			// суррогатная пара
			высокий <0?
	...
	fcssescape = function (ch, asCodePoint) {
		if (asCodePoint) {
			...
			// Управляющие символы и (в зависимости от позиции) номера экранируются как кодовые точки
			return ch.slice (0, -1) + "\\" +
...
пытаться {
	push.apply (
		(arr = slice.call (предпочитаемыйDoc.childNodes))
...
функция Sizzle (селектор, контекст, результаты, семя) {
	var m, i, elem, nid, match, groups, newSelector,
		...
		// nodeType по умолчанию 9, так как контекст по умолчанию для документа
	...
	if (! seed) {
		...
		if (documentIsHTML) {
			...
			if (support.qsa &&
				...
				// qSA учитывает элементы вне корня области видимости при оценке дочернего элемента или
				...
				// Техника должна также использоваться, когда используется ведущий комбинатор
				// как таковые селекторы не распознаются querySelectorAll.
				// Спасибо Эндрю Дюпон за эту технику.
				if (nodeType === 1 &&
					(rdescend.test (селектор) || rcombinators.test (селектор))) {

					// Расширяем контекст для селекторов одноуровневых
					newContext = rsibling.test (селектор) && testContext (context.parentNode) ||
						контекст;

					// Мы можем использовать: область вместо взлома идентификатора, если браузер
					// поддерживает это & ​​если мы не меняем контекст.
 ...
 * @param {Function} fn Пройдет созданный элемент и возвращает логический результат
 * /
function assert (fn) {
...
функция createDisabledPseudo (отключена) {
	...
	функция возврата (элемент) {
		...
		if ("форма" в элементе) {
			...
			if (elem.parentNode && elem.disabled === false) {
				...
				return elem.isDisabled === отключено ||
					...
					// Там, где нет isDisabled, проверить вручную
					/ * jshint -W018 * /
		...
		// Попытаться отсеять элементы, которые нельзя отключить, прежде чем доверять свойству disabled.
...
setDocument = Sizzle.setDocument = function (node) {
	...
	// два документа; неглубокие сравнения работают.
	...
	support.scope = assert (function (el) {
		...
		вернуть typeof el.querySelectorAll! == "undefined" &&
			! el.querySelectorAll (": div fieldset scope") .length;
	});
	...
	// так что используйте обходной тест getElementsByName
	...
	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		...
		// Стратегия регулярных выражений, принятая от Диего Перини
		assert (function (el) {
			...
			// Ничего не должно быть выбрано, когда за пустыми строками следует ^ = или $ = или * =
			...
			// IE 11 / Edge не находит элементы по запросу `[name = '']` в некоторых случаях.
			// Добавление временного атрибута в документ перед выполнением выбора
			// вокруг вопроса.
			// Интересно, что IE 10 и старше, похоже, не имеют проблемы.
			...
			// Селектор на странице `id # sibling-combinator selector` завершается неудачно
			if (! el.querySelectorAll ("a #" + expando + "+ *") .length) {
				rbuggyQSA.push (". #. + [+ ~]");
			}

			// Поддержка: только Firefox <= 3.6 - 5
			// Старый Firefox не использует плохо экранированный идентификатор.
	...
	функция (а, б) {
		...
		// Расчет позиции, если оба входа принадлежат одному и тому же документу
		// Поддержка: IE 11+, Edge 17 - 18+
		// IE / Edge иногда выдает ошибку «Отказано в доступе» при строгом сравнении
		// два документа; неглубокие сравнения работают.
		// eslint-disable-next-line eqeqeq
		...
		если (сравнить & 1 ||
			...
			// два документа; неглубокие сравнения работают.
			// eslint-disable-next-line eqeqeq
			if (a == документ || a.ownerDocument == предпочитаемый документ &&
				содержит (предпочтительный документ, а)) {
				возврат -1;
			}
			...
			// два документа; неглубокие сравнения работают.
			// eslint-disable-next-line eqeqeq
			if (b == document || b.ownerDocument == предпочитаемый документ &&
				содержит (предпочтительно Док, б)) {
				возврат 1;
			}

			// Поддерживать первоначальный заказ
			вернуть sortInput?
	...
	функция (а, б) {
		...
		вернуть я?
			...
			// Сделаем проверку родного брата, если у узлов есть общий предок
...
getText = Sizzle.getText = function (elem) {
	...
	// Не включать комментарии или узлы обработки инструкции
...
Expr = Sizzle.selectors = {
	...
	// Может быть отрегулирован пользователем
	cacheLength: 50,
	...
	preFilter: {
		...
		"РЕБЕНОК": функция (соответствие) {
			...
			* /
			...
			if (match [1] .slice (0, 3) === "nth") {
				...
				соответствует [4] = + (соответствует [4]?
					соответствует [5] + (соответствует [6] || 1):
		...
		"PSEUDO": function (match) {
			...
			// Убираем лишние символы из аргументов без кавычек
				...
				(избыток = токен (без кавычек, правда)) &&
			...
			// Возвращаем только записи, необходимые для метода псевдофильтра (тип и аргумент)
	...
	фильтр: {
		...
		"CLASS": функция (className) {
			var pattern = classCache [className + ""];

			возврат картины ||
				(pattern = new RegExp ("(^ |" + пробел +
		...
		"РЕБЕНОК": функция (тип, что, _argument, первый, последний) {
			...
			вернуть сначала === 1 && last === 0?
				...
				function (elem, _context, xml) {
					...
					if (parent) {
						...
						// non-xml: nth-child (...) сохраняет данные кэша в `parent`
						if (forward && useCache) {
							...
							if (diff === false) {
								...
								while ((node ​​= ++ nodeIndex && node && node [dir] ||
									...
									if ((ofType?
										...
										// Кэшируем индекс каждого найденного элемента
										if (useCache) {
											externalCache = узел [раскрыть] ||
												(узел [раскрыть] = {});
		...
		"PSEUDO": функция (псевдо, аргумент) {
			...
			// Помните, что setFilters наследует от псевдо
			вар аргс,
	...
	псевдо: {
		...
		"флажок": функция (элемент) {
			...
			return (имя_узла === "вход" && !! elem.checked) ||
